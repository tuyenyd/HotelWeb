...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // --- 2. Kiểm tra đúng tên token ('customerToken') ---
    const jwtToken = localStorage.getItem('customerToken');
    ...
    if (jwtToken) {
        ...
        if (userProfileMenu) userProfileMenu.style.display = 'list-item';

        // --- 4. Lấy tên từ 'currentCustomer' (đối tượng JSON) ---
        const customerDataString = localStorage.getItem('currentCustomer');
        if (userProfileName && customerDataString) {
            try {
                const customerData = JSON.parse(customerDataString);
                // Lấy tên (ưu tiên fullName, nếu không có thì dùng email)
                const displayName = customerData.fullName || customerData.email;
                ...
                userProfileName.textContent = "Tài khoản"; // Quay về mặc định nếu lỗi
            }
        }

        // --- 5. Thêm sự kiện cho nút Đăng xuất (xóa đúng key) ---
        if (logoutButton) {
        ...
        // --- 6. Nếu người dùng CHƯA ĐĂNG NHẬP ---
